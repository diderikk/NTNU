...
public:
  ...
  void move_sphere(float horizontal_position, float vertical_position) {
    transform.setOrigin(btVector3(horizontal_position * 0.25 - 0.35, vertical_position * 0.2 + 1.2, 0.0));
    falling_sphere.body.setCenterOfMassTransform(transform);
  }
  ...
  void start() {
    ...
    while (running) {
      ...
      if (ImGui::SliderFloat("Horizontal ball position", &horizontal_position, 0.0, 10.0)) {
        world.move_sphere(horizontal_position, vertical_position);
      }
